        ...
        <div class="layout-page-box">
            <el-form label-width="80px" label-position="right" :model="config" :rules="rules" ref="config"
                ...
                <el-form-item label="参数Key" prop="key">
                    <el-input v-model="config.key" placeholder="参数Key" disabled :maxlength="50"></el-input>
                </el-form-item>
    ...
    export default {
        data () {
            let keyIsExist = (rules,value,callback) => {
                if(value && value != this.originalKey){
        ...
        methods: {
            save(){
                ...
                this.$refs.config.validate((valid)=> {
                    if (valid) {
                        api.request('admin/system/config/update', data, (res)=> {
            ...
            init(){
                api.request('admin/system/config/get', {id: this.$route.query.id}, (res)=> {
                    this.originalKey = res.data.key;
                    this.config.type = res.data.type;
                    this.config.key = res.data.key;
                    this.config.name = res.data.name;
                    this.config.def = res.data.def;
                    this.config.value = res.data.value;
