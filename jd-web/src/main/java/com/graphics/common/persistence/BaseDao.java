...
import com.graphics.common.utils.NamingStrategyUtils;
import com.graphics.common.utils.Reflections;
...
//	        page.setCount(Long.valueOf(createQuery(countQlString, parameter).uniqueResult().toString()));
	...
	public List<T> findAll(){
		return getSession().createCriteria(entityClass).add(Restrictions.eq("delFlag",0)).list();
	}
	...
	@SuppressWarnings("unchecked")
	public Object findByHql(String qlString, Parameter parameter){
		Query query = createQuery(qlString, parameter);
	...
	public int update(String qlString, Parameter parameter){
		int temp =  createQuery(qlString, parameter).executeUpdate();
		return temp;
	}
	...
	public Page<T> find(Page<T> page, DetachedCriteria detachedCriteria, ResultTransformer resultTransformer) {
		...
		if (StringUtils.isNotBlank(page.getOrderBy())){
			for (String order : StringUtils.split(page.getOrderBy(), ",")){
				...
				//属性转化成驼峰命名
				String orderBy = o[0];
				orderBy = NamingStrategyUtils.camelName(orderBy);
				if (o.length==1){
